...
void	horizontal_lines_calc(t_ray *calc, t_dist *dist, t_data *display)
{
	...
	if (calc->ra > PI)
	{
		calc->ry = (((int)display->player.py / BLOCK_SIZE) * BLOCK_SIZE)
			- 0.0001;
		calc->rx = (display->player.py - calc->ry) * calc->atan
			+ display->player.px;
	...
	while (itr < display->map.height)
	{
		...
		if (calc->mx >= 0 && calc->mx < display->map.width && calc->my >= 0
			&& calc->my < display->map.height
			&& display->map.full_map_array[calc->my][calc->mx] == '1')
		{
			dist->hx = calc->rx;
			dist->hy = calc->ry;
			dist->distH = distance(display->player.px, display->player.py,
					dist->hx, dist->hy, calc->ra);
			itr = display->map.height;
		}
		else
		{
...
void	vertical_lines_calc(t_ray *calc, t_dist *dist, t_data *display)
{
	...
	{
		calc->rx = (((int)display->player.px / BLOCK_SIZE) * BLOCK_SIZE)
			- 0.0001;
		calc->ry = (display->player.px - calc->rx) * calc->ntan
			+ display->player.py;
	...
	while (itr < display->map.width)
	{
			...
			&& display->map.full_map_array[calc->my][calc->mx] == '1')
		{
			dist->vx = calc->rx;
			dist->vy = calc->ry;
			dist->distV = distance(display->player.px, display->player.py,
					dist->vx, dist->vy, calc->ra);
			itr = display->map.width;
		}
		else
		{
